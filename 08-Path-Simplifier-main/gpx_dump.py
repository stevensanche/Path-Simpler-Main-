...
#
"""
Convert a GPX file to a sequence of points in UTM form.
See documentation in instruct-prep/instructor-README.md

Original Spring 2017 by M Young; revised November 2022
"""
...
def track_centerpoint(track: list[list[float]]) -> tuple[float, float]:
...
def track_to_utm(track: list[list[float]]) -> tuple[list[tuple[int, int]], int]:
...
# A function to be called from the Python console to get UTM coordinates
# from latlon of landmark

def landmark_convert():
    """Call this interactively from the console to translate a single
    point to UTM.
    """
    zone = int(input("Integer UTM Zone >>>"))
    lat = float(input("Latitude >>>"))
    lon = float(input("Longitude >>>"))
    easting, northing, _, _ = \
        utm.from_latlon(lat, lon, force_zone_number=zone)
    print(f"UTM {round(easting)},{round(northing)}")
...
def main():
    ...
    print(f"UTM zone {utm_zone} easting, northing pairs written to {args.outfile.name}")
